...
def _td_build_from_picks(slug, display_name, field_schema, geometry_kind, api_url, token, picks):
    ...
    # Emit TethysDash's IMPORT/export format: type=dashboard + tabs[].gridItems. This is
    # what the "Import Dashboard" button (and the dashboards/add API) consume, so the user
    # imports the file directly — no hand-filling each tile's inputs. A "Main" tab holds
    # all the auto-wired tiles.
    dashboard = {
        "type": "dashboard",
        "name": display_name,
        ...
        "tabs": [{"name": "Main", "gridItems": tiles}],
    }
    ...
    with Session(engine) as session:
        ...
        if slug:
            ...
            if meta is not None:
                ...
                _tiles = dash["tabs"][0]["gridItems"]
                preview = {"slug": slug, "display_name": dn, "geometry": gk or "none",
                           "ntiles": len(_tiles), "mapping": mapping,
